...
class SmartTabStrip extends LinearLayout {
  ...
  public void setTabViewTextViewId(int tabViewTextViewId) {
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if(isSpecial){
      drawSpecialSeparator(canvas, height, tabCount);
    }else{
  ...
  private void drawSpecialSeparator(Canvas canvas,int height,int tabCount) {
    if (dividerThickness <= 0) {
      return;
    }

    final int dividerWidthPx = (int) (Math.min(Math.max(0f, dividerWidth), 1f) * 100);
    final SmartTabLayout.TabColorizer tabColorizer = getTabColorizer();
    ...
    for (int i = 0; i < tabCount - 1; i++) {
      ...
      int marginEnd = Utils.getMarginEnd(child);
      int marginStart = Utils.getMarginStart(child);
      int marginHor = Utils.getMarginHorizontally(child);

      //int separatorY = isLayoutRtl ? end - marginEnd : start + startMargin;
      dividerPaint.setColor(tabColorizer.getDividerColor(i));

      View tv = child.findViewById(tabViewTextViewId);

      canvas.drawLine(child.getRight()+marginHor/4, tv.getHeight(), child.getRight()+marginHor/4*3, tv.getHeight(), dividerPaint);
    }
  }
